...
const BlogForm: React.FC<TBlogFormProps> = ({ defaultValues, onSubmit }) => {
  ...
  return (
    <form
      ...
      {/* title error */}
      {errors.title && (
        <p className='text-red-700 text-lg'>{errors.title.message}</p>
      )}
      ...
      {/* description error */}
      {errors.description && (
        <p className='text-red-700 text-lg'>{errors.description.message}</p>
      )}
      {/* add image link input */}
      <input
        {...register("imageUrl", { required: "image url is required" })}
        value={
          "https://previews.123rf.com/images/tashatuvango/tashatuvango1703/tashatuvango170300860/73927041-my-blog-cartoon-illustration-on-purple-chalkboard-.jpg"
        }
        type='text'
      ...
      {/* imageUrl error*/}
      {errors.imageUrl && (
        <p className='text-red-700 text-lg'>{errors.imageUrl.message}</p>
      )}
      ...
      <button
        disabled={isSending ? true : false}//   disabled when isSending data === true
        type='submit'
        className='bg-zinc-700 text-slate-300 text-xl p-3 rounded-sm disabled:bg-zinc-500 disabled:cursor-not-allowed'>
